...
SwiftyperResource.prototype = {
  ...
  _streamResponseHandler(req, callback) {
    return (res) => {
      const headers = res.headers || {};
      ...
      res.once('end', () => {
        ...
        return callback.call(
          this,
          new SwiftyperAPIError({
            message: 'Expected a streaming response from the Swiftyper API',
            response,
          }),
          null
        );
  ...
  _makeHeaders(contentLength, method, userSuppliedHeaders) {
  ...
  _request(method, host, path, data, auth, options = {}, callback) {
    ...
    const makeRequest = (headers, numRetries) => {
      ...
      req.once('response', (res) => {
        ...
        } else if (options.rawStream) {
          return this._streamResponseHandler(req, callback)(res);
        } else {
    ...
    const prepareAndMakeRequest = (error, data) => {
      ...
      const headers = this._makeHeaders(
        Buffer.byteLength(requestData, 'utf-8'),
        method,
        options.headers,
        options.settings
      );
