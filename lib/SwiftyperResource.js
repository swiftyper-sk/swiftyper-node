...
SwiftyperResource.prototype = {
  ...
  _getSleepTimeInMS(numRetries, retryAfter = null) {
    const initialNetworkRetryDelay = this._swiftyper.getInitialNetworkRetryDelay();
  ...
  _request(method, host, path, data, auth, options = {}, callback) {
    ...
    const makeRequest = (headers, numRetries) => {
      ...
      const timeout = options.settings && Number.isInteger(options.settings.timeout) &&
          options.settings.timeout >= 0
            ? options.settings.timeout
            : this._swiftyper.getApiField('timeout');
      const isInsecureConnection = protocol === 'http';
      let agent = isInsecureConnection ? defaultHttpAgent : defaultHttpsAgent;
