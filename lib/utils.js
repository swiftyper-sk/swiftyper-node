...
const utils = (module.exports = {
  ...
  parseSSEStream: async function* parseSSEStream(res) {
  ...
  makeAsyncIterable: (promise) => {
    return {
      then: (...args) => promise.then(...args),
      catch: (...args) => promise.catch(...args),
      finally: (...args) => promise.finally(...args),
      [Symbol.asyncIterator]() {
        let innerIteratorPromise;
        const getInnerIterator = () => {
          if (!innerIteratorPromise) {
            innerIteratorPromise = promise.then((iterable) =>
              iterable[Symbol.asyncIterator]()
            );
          }
          return innerIteratorPromise;
        };

        return {
          async next() {
            const iterator = await getInnerIterator();
